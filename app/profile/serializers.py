...
class ProfileRequestSerializer(ModelSerializer):
    ...
    Does not allow setting the `user_id` via the request body;
    ...
    def create(self, validated_data):
        return Profile.objects.create(user_id=self.context["user"], **validated_data)


class ProfileResponseSerializer(ModelSerializer):
    """
    Serializer for returning profile data to the client.
    Includes `id` and `user_id` for transparency.
    """
