...
class WeightTrackingModelTest(TestCase):
    ...
    def _create_weight_tracking_entry(self, date, weight, user_id, notes=""):
        return WeightEntry.objects.create(date=date, weight_kg=weight, user_id=user_id, notes=notes)
    ...
    def test_weight_tracking_model_creation(self):
        ...
        saved_weight_tracking = WeightEntry.objects.get(id=weight.id)
    ...
    def test_weight_tracking_model_validation(self):
        # Try creating a WeightTracking instance with invalid measurements
        with self.assertRaises(ValidationError):
            negative_weight = -5.0  # Negative measurements is not allowed
    ...
    def test_weight_tracking_all_dates_for_user(self):
        ...
        for weight in WeightEntry.objects.filter(user_id=self.user):
