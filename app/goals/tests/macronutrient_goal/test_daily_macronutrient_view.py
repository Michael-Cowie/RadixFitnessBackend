...
from goals.urls import DAILY_MACRONUTRIENT_GOAL_NAME


def _get_entry_for_user_on_date(user, user_date):
    return DailyMacronutrientGoal.objects.get(user=user, date=user_date)
...
class DailyIntakeTrackingViewTests(APITestCase):
    ...
    def test_put_update_daily_intake_tracking_success(self):
        ...
        daily_intake_model_entry = _get_entry_for_user_on_date(self.user, updated_data["date"])

        self.assertEqual(daily_intake_model_entry.user, self.user)
    ...
    def test_put_create_new_daily_intake_tracking_success(self):
        ...
        daily_intake_model_entry = _get_entry_for_user_on_date(self.user, new_data["date"])
